...
function biomedicalengineering() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      setIsLoading(true);
      setError(null);
      const collectionName1 = 'BiomedicalEngineeringProjects(Year 4)';
      const collectionName2 = 'BiomedicalEngineeringProjects(Year 5)';  
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_NGROK_URL}/projects2?collection1=${encodeURIComponent(collectionName1)}&collection2=${encodeURIComponent(collectionName2)}&page=${currentPage}&limit=${projectsPerPage}`,
        {
          method: 'GET', 
          headers: {
            'ngrok-skip-browser-warning': 'any-value' 
  ...
  return (
    ...
    <div className="flex min-h-screen bg-expo-blue">
      ...
      <div className="flex-1 p-4">
        ...
        <h1 className="text-center text-3xl font-bold my-6">Biomedical Engineering</h1>
