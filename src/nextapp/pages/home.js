...
function Home() {
  ...
  const [isLoading, setIsLoading] = useState(true); // Default to true to handle initial load
  const [error, setError] = useState(null);
  const [currentPage, setCurrentPage] = useState(1);
  const [projectsPerPage] = useState(20);
  const { data: session, status } = useSession(); // Destructure status for loading checks
  const router = useRouter();

  useEffect(() => {
    if (status === "loading") {
      setIsLoading(true);
      return; // Keep showing loading until the status is confirmed
    }
    // Redirect the user if not signed in
    if (status === "unauthenticated") {
      signIn(); // Redirect user to the sign-in page
    }
  }, [status]);
  ...
  useEffect(() => {
    if (status === "authenticated") {
      const fetchProjects = async () => {
        ...
        setError(null);
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_NGROK_URL}/allprojects?page=${currentPage}&limit=${projectsPerPage}`,
          {
            method: 'GET', // Method is GET by default, but it's good to be explicit
            headers: {
  ...
  }, [currentPage, projectsPerPage, status]); // Include status to re-run when authentication status changes
  if (isLoading) {
    return (
