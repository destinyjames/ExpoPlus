...
function datascience() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      setIsLoading(true);
      setError(null);
      const collectionName = 'DataScienceProjects'; // Define the collection name
      try {
        const ngrokUrl = process.env.NEXT_PUBLIC_NGROK_URL; // Ensure this is defined in your .env.local file
        const response = await fetch(`${ngrokUrl}/projects?collection=${encodeURIComponent(collectionName)}&page=${currentPage}&limit=${projectsPerPage}`, {
          method: 'GET',
          headers: {
            'ngrok-skip-browser-warning': 'any-value' 
          },
  ...
  return (
    ...
    <div className="flex min-h-screen bg-expo-blue">
      ...
      <div className="flex-1 p-4">
        ...
        <h1 className="text-center text-3xl font-bold my-6">Data Science</h1>
